...
CREATE OR REPLACE FUNCTION spAI_Get_Menu_Tasks(pstatus INTEGER) RETURNS SETOF tpRecordMenuTask AS $PROC$
	...
	BEGIN
		...
		FOR datarow IN EXECUTE ('SELECT id, client_id, task, status, message FROM menu_tasks WHERE EXTRACT(EPOCH FROM(CURRENT_TIMESTAMP-created_at)) > 30 AND status=' || pstatus ) LOOP
			RETURN NEXT datarow;
		END LOOP;
	END;
