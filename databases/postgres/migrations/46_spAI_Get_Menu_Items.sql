...
CREATE OR REPLACE FUNCTION spAI_Get_Menu_Items(pcustomer_id CHAR(36), pprompt VARCHAR, pembeddings VARCHAR) RETURNS SETOF tpRecordSearchInputOutput AS $PROC$
	...
	BEGIN
		...
		ELSE
			...
			ELSE
				SELECT id INTO promptId FROM menu_prompts WHERE customer_id=pcustomer_id AND embeddings=withEmbeddings AND original_prompt=pprompt AND deleted_at IS NULL;
				--RAISE WARNING 'promptFirstAsk:%', promptFirstAsk;
				IF promptFirstAsk=true OR promptFirstAsk='t' THEN
					pprompt := modifiedPrompt;
					INSERT INTO menu_searches(prompt_id, content_id, product_name_bm25, product_id_bm25, product_category_bm25, total_bm25, similarity)
